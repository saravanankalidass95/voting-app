<div wire:poll="getNotificationCount" x-data="{ isOpen: false }" class="relative">
    ...
    ">
        ...
        @if ($notificationCount)
            <div class="absolute rounded-full bg-red text-white text-xxs w-6 h-6 flex justify-center items-center border-2 top-0 right-0 -top-2 -right-2">{{ $notificationCount }}</div>                                    
        @endif
    </button>
    <ul
        class="absolute w-76 md:w-96 text-left text-gray-700 text-xs bg-white border shadow-dialog z-10 rounded-xl max-h-128 overflow-y-auto -right-28 md:-right-12"
        {{-- style="right: -46px" --}}
        x-cloak
    ...
    >
        ...
        @else
            <li class="mx-auto w-40 py-6">
                <img src="{{ asset('img/no-ideas.svg') }}" all="No Ideas" class="mx-auto" style="mix-blend-mode luminosity">
                <div class="class text-gray-400 text-center font-bold mt-6">No new notifications</div>
